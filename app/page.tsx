...
type Recipe = { name:string; emoji:string; tone?:string; time:number; cost:number; protein:number; description:string; ingredients:Ingredient[]; steps:string[]; note:string; source?:"ai"|"local" };
...
export default function Home(){
  const [tab,setTab]=useState<"home"|"pantry"|"list">("home");
  ...
  function normalize(raw:Recipe,index:number):Recipe{return{...raw,tone:COLORS[index%COLORS.length],source:"ai",time:Number(raw.time)||20,cost:Number(raw.cost)||3,protein:Number(raw.protein)||15,description:raw.description||"用手边食材完成的一道新灵感。",ingredients:Array.isArray(raw.ingredients)?raw.ingredients:[],steps:Array.isArray(raw.steps)?raw.steps:[],note:raw.note||"烹饪前确认食材新鲜并彻底加热。"}}
  async function generate(){
    if(!pantry.length){setNotice("先往冰箱里放点食材吧。");return}setLoading(true);setNotice("");setChosen(null);setChecked([]);
    try{const response=await fetch("/api/recommend",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pantry,minutes,goal,people})});const data=await response.json() as {recipes?:Recipe[];error?:string;message?:string};if(!response.ok)throw new Error(data.message||data.error||"推荐失败");setRecipes((data.recipes||[]).map(normalize));setNotice("AI 已根据你输入的全部食材创作了 3 道新方案。");}
  ...
  function selectRecipe(recipe:Recipe){setChosen(recipe);setChecked([]);setNotice(`已选定「${recipe.name}」，购物清单只为这道菜服务。`);setTimeout(()=>document.getElementById("commitment")?.scrollIntoView({behavior:"smooth"}),50)}
  ...
  return <main className="app-shell">
    <header className="topbar"><button className="brand" onClick={()=>setTab("home")} aria-label="返回首页"><span className="brand-mark" aria-hidden="true"><span className="cloche-icon" /></span><span><b>BECOME A CHEF</b><small>COOK WITH WHAT YOU HAVE</small></span></button><div className="header-actions"><span className="weather">COOK WITH WHAT YOU HAVE</span><button className="avatar" aria-label="品牌标记">B</button></div></header>
    <section className="content">
      {tab==="home"&&<>
        ...
        <div className="planner-card"><div className="field-head"><label htmlFor="ingredients">你的食材台</label><span>{pantry.length} 种食材</span></div><form className="ingredient-box" onSubmit={addItems}><div className="chips">{pantry.slice(0,10).map(item=><button type="button" className="chip" key={item} onClick={()=>setPantry(pantry.filter(x=>x!==item))}>{item}<span>×</span></button>)}<input id="ingredients" value={draft} onChange={e=>setDraft(e.target.value)} placeholder="任何食材都可以，按回车添加…"/></div><button className="add-button" aria-label="添加食材">＋</button></form><div className="quick-add"><span>试试添加</span>{["面条","鸡肉","土豆","菠菜","豆腐"].filter(x=>!pantry.includes(x)).slice(0,4).map(item=><button key={item} onClick={()=>setPantry([...pantry,item])}>＋ {item}</button>)}</div>
          <div className="preference-grid"><div className="preference"><span className="pref-label">⏱ 可用时间</span><div className="segment">{[15,30,45].map(n=><button type="button" className={minutes===n?"active":""} onClick={()=>setMinutes(n)} key={n}>{n} 分钟</button>)}</div></div><div className="preference"><span className="pref-label">✦ 创作方向</span><div className="segment goal">{GOALS.map(x=><button type="button" className={goal===x?"active":""} onClick={()=>setGoal(x)} key={x}>{x}</button>)}</div></div><div className="preference people"><span className="pref-label">♙ 用餐人数</span><div className="counter"><button type="button" onClick={()=>setPeople(Math.max(1,people-1))}>−</button><b>{people}</b><button type="button" onClick={()=>setPeople(Math.min(6,people+1))}>＋</button></div></div></div>
          <button className="decide" disabled={loading} onClick={generate}><span>{loading?"主厨正在构思…":"给我三道创作灵感"}</span><span>{loading?"◌":"→"}</span></button>
        </div><div className="trust-row"><span>✓ 接受任意合理食材</span><span>✓ 只推荐真正能做的菜</span><span>✓ 选定后才生成采购清单</span></div>
        {!!recipes.length&&<section id="results" className="results"><div className="section-title"><div><span className="eyebrow">CHEF&apos;S IDEAS</span><h2>今天可以这样创作</h2></div><button onClick={()=>setRecipes([])}>收起灵感</button></div>{notice&&<div className="notice">✦ {notice}</div>}<div className="recipe-grid">{recipes.map((recipe,index)=>{const owned=recipe.ingredients.filter(i=>pantry.includes(i.name)).length;const missing=recipe.ingredients.filter(i=>i.required&&!pantry.includes(i.name));return <article className={`recipe-card ${chosen?.name===recipe.name?"chosen":""}`} key={recipe.name}><div className={`food-visual ${recipe.tone}`}><span>{recipe.emoji||"🍽️"}</span><b>{recipe.source==="ai"?"AI 新灵感":index===0?"最合适":"经典搭配"}</b></div><div className="recipe-body"><div className="recipe-kicker">{recipe.description}</div><h3>{recipe.name}</h3><div className="metrics"><span>⏱ {recipe.time} 分钟</span><span>约 ${recipe.cost.toFixed(1)}</span><span>{recipe.protein}g 蛋白质</span></div><div className="match"><span style={{width:`${Math.round(owned/Math.max(1,recipe.ingredients.length)*100)}%`}}/><small>已有 {owned}/{recipe.ingredients.length} 种 · {missing.length?`核心缺 ${missing.length} 种`:"核心食材齐全"}</small></div><p>{missing.length?<>要完成还需要：<b>{missing.map(i=>i.name).join("、")}</b></>:<b className="ready">食材齐全，现在就能做</b>}</p><button className="choose" onClick={()=>selectRecipe(recipe)}>{chosen?.name===recipe.name?"✓ 已选这道":"就做这道"}<span>→</span></button></div></article>})}</div></section>}
        {chosen&&<section id="commitment" className="commitment"><div><span className="eyebrow">YOUR DISH</span><h2>决定了：{chosen.name}</h2><p>{mustBuy.length?`再准备 ${mustBuy.length} 样核心食材，就可以开火。`:"核心食材已经齐全，直接开始。"}</p></div><div className="commit-actions"><button onClick={()=>setTab("list")}>查看这道菜的清单</button><button className="cook-now" onClick={()=>startCooking(chosen)}>开始烹饪 →</button></div></section>}
      </>}
      {tab==="pantry"&&<section className="subpage"><span className="eyebrow">YOUR STATION</span><h1>食材台</h1><p>这里是你的创作起点。点击食材即可移除，记录只保存在当前设备。</p><form className="pantry-add" onSubmit={addItems}><input value={draft} onChange={e=>setDraft(e.target.value)} placeholder="添加任何食材…"/><button>放上食材台</button></form><div className="pantry-list">{pantry.map((item,i)=><button key={item} onClick={()=>setPantry(pantry.filter(x=>x!==item))}><span>{["🥚","🍅","🍚","🧅","🥛","🍜","🍗","🥔","🥬"][i%9]}</span><b>{item}</b><small>可用于下一次创作</small><i>×</i></button>)}</div></section>}
      {tab==="list"&&<section className="subpage"><span className="eyebrow">FOR ONE CHOSEN DISH</span><h1>备料清单</h1>{!chosen?<div className="empty-state"><span>📝</span><h2>先决定做哪一道</h2><p>清单不应该替三个候选菜同时买单。选定一道菜后，这里才会列出它真正缺少的东西。</p><button onClick={()=>setTab("home")}>返回选择一道菜</button></div>:<><p>只为「{chosen.name}」准备。发现家里其实有？点“已有”即可从清单移除。</p><div className="shopping-card"><div className="shopping-head"><span>🧺</span><div><b>{chosen.name}</b><small>{mustBuy.length} 样必需 · {upgrades.length} 样可选</small></div></div><h3 className="list-group">必须准备</h3>{mustBuy.length?mustBuy.map(item=><div className={`shop-row ${checked.includes(item.name)?"done":""}`} key={item.name}><input type="checkbox" checked={checked.includes(item.name)} onChange={()=>setChecked(checked.includes(item.name)?checked.filter(x=>x!==item.name):[...checked,item.name])}/><span><b>{item.name}</b><small>{item.amount}</small></span><button onClick={()=>markAsOwned(item.name)}>家里有</button></div>):<div className="empty compact">✓ 必需食材已经齐全</div>}{!!upgrades.length&&<><h3 className="list-group optional">可选升级 · 不买也能做</h3>{upgrades.map(item=><div className="shop-row muted" key={item.name}><span className="optional-mark">＋</span><span><b>{item.name}</b><small>{item.amount}</small></span><button onClick={()=>markAsOwned(item.name)}>家里有</button></div>)}</>}<div className="list-footer"><button onClick={()=>startCooking(chosen)}>食材准备好了，开始烹饪 →</button></div></div></>}
      </section>}
    </section>
    <nav className="bottom-nav" aria-label="主要导航"><button className={tab==="home"?"active":""} onClick={()=>setTab("home")}><span>✦</span>创作灵感</button><button className={tab==="pantry"?"active":""} onClick={()=>setTab("pantry")}><span>▣</span>食材台</button><button className={tab==="list"?"active":""} onClick={()=>setTab("list")}><span>✓</span>备料清单{chosen&&mustBuy.length>0&&<i>{mustBuy.length}</i>}</button></nav>
