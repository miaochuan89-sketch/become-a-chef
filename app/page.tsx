...
export default function Home(){
  ...
  async function generate(){
    ...
    try{const response=await fetch("/api/recommend",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pantry,minutes,goal})});const data=await response.json() as {recipes?:Recipe[];source?:"ai"|"hybrid"|"fallback";error?:string;message?:string};if(!response.ok)throw new Error(data.message||data.error||"推荐失败");const next=(data.recipes||[]).map(normalize);setRecipes(next);setNotice(data.source==="ai"?"AI 已生成并通过规则校验的 3 道菜谱。":data.source==="hybrid"?"AI 方案已通过合理性筛选，不足的部分用经典菜谱补齐。":`AI 暂时没有返回可靠结果，先提供 ${next.length} 道真正匹配食材的经典菜谱。`);}
