...
export default function CommunityFeed({ recipeName }:{ recipeName?:string }) {
  ...
  async function publish(event:FormEvent){event.preventDefault();if(!image){setStatus("请先选择一张成品照片。");return}setPublishing(true);setStatus("");const form=new FormData();form.set("author",author);form.set("caption",caption);form.set("image",image);if(recipeName)form.set("recipeName",recipeName);try{const response=await fetch("/api/posts",{method:"POST",body:form});const data=await response.json() as {error?:string};if(!response.ok)throw new Error(data.error||"发布失败");setAuthor("");setCaption("");setImage(null);if(fileRef.current)fileRef.current.value="";setStatus("发布成功，已经出现在 Chef’s Table。");await loadPosts()}catch(error){setStatus(error instanceof Error?error.message:"发布失败，请稍后再试。")}finally{setPublishing(false)}}
