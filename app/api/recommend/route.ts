...
export async function POST(request: NextRequest) {
  ...
  if (usage && usage.reset > now && usage.count >= 10) {
    return NextResponse.json({ error: "今天先消化一下这些灵感吧，稍后再试。" }, { status: 429 });
  }
  windows.set(ip, !usage || usage.reset <= now ? { count: 1, reset: now + 60 * 60 * 1000 } : { ...usage, count: usage.count + 1 });

  const body = await request.json().catch(() => null) as null | { pantry?: unknown; minutes?: unknown; goal?: unknown; people?: unknown };
  ...
  const people = Math.min(8, Math.max(1, Number(body?.people) || 1));
  ...
  const prompt = `你是严谨又有创意的家庭主厨。根据用户现有食材设计恰好3道真正可执行、彼此风格不同的菜。\n现有食材：${pantry.join("、")}\n人数：${people}\n期望时间：${minutes}分钟以内\n目标：${goal}\n要求：\n1. 尽量多用现有食材，但绝不为了使用奇怪食材而设计难吃或危险的组合。\n2. 如果输入不是食物、可能有毒、已变质或生食有风险，不得使用，并在note解释。\n3. 每道菜列出全部食材和${people}人份具体用量；基础油盐水可以列出但标为required=false。\n4. required=true仅用于缺了就无法完成这道菜的核心食材；点缀、升级风味和可替代调料标为false。\n5. 步骤清楚，包含关键火候与食品安全提醒。成本为美元估算，protein为整道菜每人蛋白质克数。\n6. 只返回合法JSON，不要Markdown。格式严格为：${schema}`;
  ...
  return NextResponse.json({ error: "AI_SERVICE_UNAVAILABLE", message: "主厨服务暂时拥挤，请点一下再试。" }, { status: 503 });
}
